...
float traceShadow(vec3 origin, vec3 lightPos) {
    ...
    float dist = length(L);
    L = normalize(L);
    vec3 originOffset = origin + L * 0.01; // Offset to clear acne
...
void sampleLights(vec3 hit_pos, vec3 N, vec3 V, vec3 albedo, float roughness, float metallic, float transmission, inout vec3 Lo) {
    // [Insert your existing sampleLights logic here - Assumed correct]
    // Ensure you use shadowPayload logic consistent with traceShadow above.
    
    // BASIC FALLBACK if you haven't pasted it back:
    uint num_lights = light_cdf.entries.length();
    if (num_lights == 0) return;
    // ... [CDF selection code] ...
    // ... [Triangle pos calculation] ...
    // if (traceShadow(hit_pos, light_pos) > 0.0) Lo += ...
}
...
void main()
{
    ...
    if (transmission > 0.0) {
        ...
        if (rnd(payload.seed) < prob_reflect) {
            ...
            float eta = (dot(N_geo, V) > 0.0) ? (1.0 / 1.5) : 1.5;
            vec3 N_refr = (dot(N_geo, V) > 0.0) ? N : -N;
    ...
    // Opaque PBR (Diffuse vs Specular)
