...
// --- UNIFORMS (Identical to fragment.frag) ---
...
// --- FRAGMENT PUSH CONSTANT (Identical to fragment.frag) ---
...
// --- INPUTS (Identical to fragment.frag) ---
...
// --- OIT OUTPUTS ---
layout(location = 0) out vec4 outAccum;   // Accumulation (R16G16B16A16_SFLOAT)
layout(location = 1) out vec4 outReveal;  // Revealage (R16_SFLOAT)

// --- PBR Constants and Functions (Identical to fragment.frag) ---
...
// --- Main Function ---
void main() {
    // --- 1. Get Material Properties (Identical to fragment.frag) ---
    ...
    // --- 2. Calculate Base Vectors (Identical to fragment.frag) ---
    vec3 V = normalize(ubo.cameraPos - fragWorldPos);
    vec3 N;
    // ... (copy normal map logic) ...
    if(fragInTangentW != 0){
    ...
    // --- 5. Calculate Lighting (Identical to fragment.frag) ---
    ...
    // --- 6. Final Color (MODIFIED FOR OIT) ---
    vec3 ambient = vec3(0.01) * albedo * ao;
    vec3 color = ambient + Lo + emissive; // This is our final linear HDR color

    // DO NOT TONE MAP OR GAMMA CORRECT
    ...
    // 2. Set outputs
    // Accumulation buffer stores weighted, premultiplied color
    outAccum.rgb = premultiplied_color * weight;
    outAccum.a = alpha * weight;
    
    // Revealage buffer stores transmittance (1 - alpha)
    // We only need the .r channel
    outReveal.r = max(1.0-alpha, 1e-4);
    outReveal.gba = vec3(0.0);
}