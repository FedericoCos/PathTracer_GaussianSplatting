...
void sampleLights(vec3 hit_pos, vec3 N, vec3 V, vec3 albedo, float roughness, float metallic, float transmission, inout vec3 Lo) 
{
    ...
    uint left = 0; uint right = num_lights - 1; uint idx = 0;
    while (left <= right) {
...
void main()
{
    ...
    vec3 N_geo = normalize(vec3(gl_ObjectToWorldEXT * vec4(normal_obj, 0.0))); 
    vec3 T_geo = normalize(vec3(gl_ObjectToWorldEXT * vec4(tangent_obj.xyz, 0.0))); 
    ...
    // Requires updated MaterialData struct
    ...
    // --- 4. TRANSPARENCY & MASK LOGIC (PRESERVED) ---
    ...
    // OPAQUE PATH (FIXED FOR CLEARCOAT)
    payload.next_ray_origin = hit_pos + N_geo * 0.001;

    // [FIX] Valid Clearcoat Logic
    ...
    if (cc_valid) {
        ...
        // Probability of hitting coat depends on Fresnel strength and Coat factor
    ...
    if (cc_valid && rnd(payload.seed) < prob_cc) {
        ...
        // Value approx (F_cc * clearcoat). Prob approx (F_cc * clearcoat). 
        payload.weight = vec3(1.0); 
    } 
    else {
        // --- HIT BASE LAYER ---
        // Apply energy conservation for light lost to clearcoat reflection
        vec3 energy_attenuation = vec3(1.0);
        if (cc_valid) { 
             float prob_base = max(1.0 - prob_cc, 0.001);
             // Energy lost to coat reflection is (F_cc * clearcoat)
